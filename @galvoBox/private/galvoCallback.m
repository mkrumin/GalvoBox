...
try
    ...
    warning('The following error caught in galvoCallback:\n');
    warning(e.message);
    % even when catching the error the DataGramReceivedFcn gets disabled
    % sometimes (after 2-3 times)
    % preemptively re-enabling the callback
    fclose(obj.galvoUDP); 
    fopen(obj.galvoUDP)
    snd = load('laughter');
    sound(snd.y, snd.Fs);
